 ...
 * @param {array<object>} members.testSuites
 * @param {array<object>} members.testCases
 * @param {array<object>} members.testSteps
...
function buildNav(members) {
  ...
  // nav += buildMemberNav(members.testSuites, 'Test Suites', seen, linkto);

  if (members.testSuites.length) {
    testSuiteNav = ''
    members.testSuites.forEach(({ kind, name }) => {
      if (kind === 'testsuite') {
        testSuiteNav += `<li>${linkto(name, name)}</li>`
      }
    })
  }
  if (members.globals.length) {
    ...
    /*
    Removed this to avoid having any other files other than automated
    tests, we can switch it if we need it for other documentation 
    */
    /* 
     if (!globalNav) {
    ...
    if (testSuiteNav) {
      nav += `<h3>Test Suites</h3><ul>${testSuiteNav}</ul>`
    }
  }
...
exports.publish = (taffyData, opts, tutorials) => {
  ...
  let testcases
  let teststeps
  const testsuiteDoclets = []
  const testcasesDoclets = []
  const teststepsDoclets = []
  ...
  data().each((doclet) => {
    if (doclet.kind === 'testsuite') {
      const url = helper.getUniqueFilename(doclet.name)
      helper.registerLink(doclet.name, url)
    }
  })
  ...
  data().each((doclet) => {
    ...
    if (doclet.kind === 'testsuite') {
      testsuiteDoclets.push(doclet)
    }
    if (doclet.kind === 'testcase') {
      testcasesDoclets.push(doclet)
    }
    if (doclet.kind === 'teststep') {
      teststepsDoclets.push(doclet)
    }
  })

  members = helper.getMembers(data)
  members.tutorials = tutorials.children
  members.testSuites = testsuiteDoclets
  members.testCases = testcasesDoclets
  members.testSteps = teststepsDoclets
  ...
  testsuites = taffy(members.testSuites)
  testcases = taffy(members.testCases)
  teststeps = taffy(members.testSteps)
  ...
  Object.keys(helper.longnameToUrl).forEach((longname) => {
    ...
    const myTestSuites = helper.find(testsuites, {
      longname: longname
    })
    ...
    if (myTestSuites.length) {
      for (let i = 0; i < myTestSuites.length; i++) {
        generate(
          `Test Suite: ${myTestSuites[0].name}`,
          myTestSuites,
          myTestSuites[0].name + '.html'
        )
      }
    }
  })
